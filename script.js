// ==================== BACKGROUND MUSIC - IMMEDIATE PLAY ====================
...
if (music) {
  ...
  // Fallback: try play on window load
  window.addEventListener("load", () => {
    if (music.paused) {
      music.play().catch(error => {
        console.log("Load event autoplay also blocked");
        if (btn) btn.style.display = "inline-block";
      });
    }
  });

  // Button click to play music if autoplay blocked
  if (btn) {
